...
| SisdaiModal                | Ventana emergente de información variada que bloquea el resto del contenido de la vista hasta que se cierre                                                     | tamanio="`nombre de clase`" (el nombre de la clase puede ser chico, mediano, grande, pantalla-completa)                                        | `<template v-slot:encabezado>` `<template v-slot:cuerpo>` `<template v-slot:pie>` |
